...
#ifdef SYNO_USBBOOT
		if ((SYNO_USBMAN_ID_VENDOR == dev->descriptor.idVendor &&
			 SYNO_USBMAN_ID_PRODUCT == dev->descriptor.idProduct)) {
			char *env;

			printf("Detected manufacturing device... Entering manufacturing mode...\n");

			env = getenv("bootargs");
			if (!env) {
				printf("Failed to get bootargs, skip to enter manufacturing mode.\n");
				continue;
		}
			syno_bootargs_replace(env, "root", "/dev/ram"); // watch out buffer size
			continue;
		}
		if (!(SYNO_USBBOOT_ID_VENDOR == dev->descriptor.idVendor &&
			  SYNO_USBBOOT_ID_PRODUCT == dev->descriptor.idProduct)) {
				/* Skip non-boot disks */
				continue;
		}
#endif /* SYNO_USBBOOT */
